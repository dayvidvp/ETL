...
def sorted_dict_str(data: JsonType) -> StrTreeType:
    if type(data) == dict:
        result = {k: sorted_dict_str(data[k]) for k in sorted(data.keys())}
        # logger.debug("hash generated")
        return result
    elif type(data) == list:
        result = [sorted_dict_str(val) for val in data]
        # logger.debug("hash generated")
        return result
    else:
        return str(data)
...
def get_json_sem_hash(data: JsonTree, hasher=hashlib.sha256) -> str:
    ...
    result = hasher(bytes(repr(sorted_dict_str(data)), 'UTF-8')).hexdigest()
    # logger.debug(f"hash generated: {result}")
